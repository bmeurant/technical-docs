...
-   **API Complexity**: While simpler for the client, GraphQL shifts a lot of complexity to the server. Implementing a performant GraphQL server with features like batching (e.g., via DataLoader) to solve the [[system-performance#3. Extraneous Fetching|N+1 problem]] requires careful design.
