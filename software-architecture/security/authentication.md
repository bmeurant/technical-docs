...
*   **Mechanism:** After initial authentication (e.g., with a username/password), the server issues a cryptographic token (e.g., a JSON Web Token - JWT) to the client. The client then includes this token with subsequent requests to prove its identity without re-sending credentials. Tokens are often stateless on the server side, improving scalability.
*   **Examples:** [[jwt|JSON Web Tokens (JWT)]], session tokens.

### 3. OAuth (Open Authorization) and OpenID Connect (OIDC)
